...
namespace SimsCCManager.Packages.Sorting
{
    ...
    public class FilesSort
    {
        ...
        public void SortPackage(SimsPackage package){
            ...
            if (thisPackage.Type == "Merged Package"){
                ...
                GlobalVariables.RemoveSimsPackage.Enqueue(thisPackage);
        ...
        public void MoveFile(List<AllFiles> files){

            var ts4Script = files.Where(f => f.Type == "ts4script");
            var ts3pack = files.Where(f => f.Type == "sims3pack");
            var ts2pack = files.Where(f => f.Type == "sims2pack");
            var comp = files.Where(f => f.Type == "compressed file");
            var tray = files.Where(f => f.Type == "tray file");
            var other = files.Where(f => f.Type == "other");            

            foreach (var file in ts4Script) {
                if (File.Exists(file.Location)){
                    string newloc = Path.Combine(S4ScriptsFolder, file.Name);
                    if (!Directory.Exists(S4ScriptsFolder)){
                        Methods.MakeFolder(S4ScriptsFolder);                        
                    }
                    if (File.Exists(file.Location)){
                        File.Move(file.Location, newloc);
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
                        file.Location = newloc;
                        GlobalVariables.AddAllfiles.Enqueue(file);
                    } else {
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
                    }
                }
            ...
            foreach (var file in ts3pack) {
                if (File.Exists(file.Location)){
                    string newloc = Path.Combine(S3PacksFolder, file.Name);
                    if (!Directory.Exists(S3PacksFolder)){
                        Methods.MakeFolder(S3PacksFolder);                        
                    }
                    if (File.Exists(file.Location)){
                        File.Move(file.Location, newloc);
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
            ...
            foreach (var file in ts2pack) {
                if (File.Exists(file.Location)){
                    string newloc = Path.Combine(S2PacksFolder, file.Name);
                    if (!Directory.Exists(S2PacksFolder)){
                        Methods.MakeFolder(S2PacksFolder);                        
                    }
                    if (File.Exists(file.Location)){
                        File.Move(file.Location, newloc);
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
            ...
            foreach (var file in comp) {
                if (File.Exists(file.Location)){
                    string newloc = Path.Combine(ZipFilesFolder, file.Name);
                    if (!Directory.Exists(ZipFilesFolder)){
                        Methods.MakeFolder(ZipFilesFolder);                        
                    }
                    if (File.Exists(file.Location)){
                        File.Move(file.Location, newloc);
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
            ...
            foreach (var file in tray) {
                if (File.Exists(file.Location)){
                    string newloc = Path.Combine(TrayFilesFolder, file.Name);
                    if (!Directory.Exists(TrayFilesFolder)){
                        Methods.MakeFolder(TrayFilesFolder);                        
                    }
                    if (File.Exists(file.Location)){
                        File.Move(file.Location, newloc);
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
            ...
            foreach (var file in other) {
                if (File.Exists(file.Location)){
                    string newloc = Path.Combine(NonSimsFiles, file.Name);
                    if (!Directory.Exists(NonSimsFiles)){
                        Methods.MakeFolder(NonSimsFiles);                        
                    }
                    if (File.Exists(file.Location)){
                        File.Move(file.Location, newloc);
                        GlobalVariables.RemoveAllfiles.Enqueue(file);
